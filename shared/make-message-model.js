...
export function isExecutableMessage(message) { const value = migrateMakeMessage(message); return value.mode !== "ask" && !(value.questions.length && !value.improvedPrompt) && Boolean(text(message?.executablePrompt || message?.finalPrompt || value.improvedPrompt || value.content)); }
