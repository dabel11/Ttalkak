...
function App() {
  ...
  const [ragConfig, setRagConfig] = useState(() => {
    ...
    return {
      collectionName: MODE_META[stored?.collectionName]
        ? stored.collectionName
        : DEFAULT_RAG_CONFIG.collectionName,
    };
  });
  ...
  async function submitPrompt() {
    ...
    try {
      const data = await improvePrompt({
        prompt,
        category: ragConfig.collectionName,
        conversationId:
          typeof activeThreadId.current === "number"
      ...
      if (serverThreadId != null) {
        activeThreadId.current =
          Number(serverThreadId);
      } else if (!activeThreadId.current) {
        activeThreadId.current =
          `local-${Date.now()}`;
      }
        ...
        ...previous,
        {
          ...
          content:
            error instanceof TypeError
              ? "Spring Boot Backend에 연결할 수 없습니다. 서버 실행 상태와 API 주소를 확인해 주세요."
              : `요청 처리 중 오류가 발생했습니다.\n\n${error.message}`,
...
function RagSettingsPanel({
  ...
  const [local, setLocal] = useState({
    collectionName:
      config.collectionName || "prompt_techniques",
  });
  ...
  return (
    <div
      className="rag-settings-panel"
      role="dialog"
      aria-label="RAG 모드 설정"
    >
      <div className="rag-settings-header">
        <span>RAG 모드 설정</span>

        <span className={`rag-status ${status}`}>
      ...
      >
        <label>
          ...
          >
            <option value="prompt_techniques">
              기법 모드
            </option>
