...
### 🔴 발견한 문제 — judge 포화 (→ v2에서 수정함)

context_retention 2항목은 **근거를 아예 빼도(rag_off) 만점 5.00**이었습니다.

원인은 채점자가 후해서가 아니라 **루브릭에 검색이 움직일 수 있는 축이 없어서**였습니다.
기존 4개 기준(문맥 유지·지시 준수·명확성·사실 충실도)은 전부 기법 카드 없이도
만족 가능하니, 검색을 켜든 끄든 구조적으로 같은 점수가 나옵니다.
...
| `mustInclude` | 입력에만 있고 사용 지시 없음 | **누락 1건당 1점 감점** |
| `mustNotInclude` | 동일 | **1건이라도 나오면 최대 2점** |
| 채점 절차 | 없음 | 결함 먼저 찾고 → 점수 → 근거에 감점 사유 |

`JUDGE_PROMPT_VERSION`을 v2로 올려 **예전 점수가 캐시에서 조용히 재사용되지 않습니다.**

첫 검증에서 `context_retention_01` / rag_off가 **5.00 → 4.80**으로 내려왔습니다.
천장은 깨졌지만 n=1이라 방향 신호입니다.
