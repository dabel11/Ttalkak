...
- [ ] 🔴 **gen_eval 전량 재측정 + judge 확보** — 지난 측정은 완주 13/18(생성 백엔드 양쪽 소진), 환각률 미확보(judge 429). 캐시 `eval/.cache_v3.json` 에 완주분이 남아 있어 **누락분 생성 + 판정만** 채우면 된다.
...
## [2026-07-31] gen_eval end-to-end 재측정 (2단계 파이프라인, HyDE off)
**목적**: 규약 v3(분석기+하이브리드) 적용 후 실제 `/query` 경로의 mode 판정·생성 품질·환각을 재측정.

**조건**: `gen_eval --no-hyde`, gen_set 18문항, 생성 temp 0.7, judge llama-3.3-70b(temp 0).

**결과**
| 지표 | 종전(단일 단계·HyDE on) | 이번(2단계·HyDE off) |
|---|---|---|
| mode_accuracy | 0.78 (14/18) | **1.00 (13/13 완주분)** |
| structured(JSON 파싱) | 18/18 | 13/13 (폴백 0회) |
| technique_grounding | 4.33 (n=18) | 4.00 (n=1 — judge 굶음) |
| faithfulness·환각률 | 5.00 / 0.00 (n=7) | **측정 실패** (judge 429) |

- ⭐ **mode_accuracy 0.78 → 1.00**: 2단계(분석기가 필드 상태를 먼저 확정)의 효과가 end-to-end 에서 확인됨. 종전에 흔들리던 borderline(제주도·회의록·환불 이메일 등)이 전부 기대대로.
- ⚠️ **완주 13/18**: 나머지 5건(11·14·15·17·18)은 **생성 백엔드 양쪽 소진**으로 실패 — Groq 70b TPD + Gemini 일일 한도. 2단계라 요청당 호출이 1회 늘어난 것이 쿼터 소모를 앞당긴 것으로 보인다.
- ⚠️ **따라서 1.00 은 완주분에 한한 값**이다. 누락 5건에는 분석기 전수조사에서 유일하게 틀렸던 `"이거 좀 개선해줘"`(11)가 포함돼 있어, 전량 측정 시 1.00 보다 낮을 가능성이 높다.
- ⚠️ **환각률은 이번에 못 얻었다**(judge 429). 다만 핵심 사례는 **라이브 3회 반복으로 직접 확인**: 신제품 이어폰 요청에서 3/3 improve + 빈칸 3~8개 생성, 창작 스펙(30시간·IPX·AirSound) **0건**.

**결정·근거**
- 쿼터 회복 후 **전량 재측정 + judge 확보**가 필요(백로그). 캐시(`eval/.cache_v3.json`)에 완주분 생성이 남아 있어 재실행 시 누락분·판정만 채우면 된다.
- 2단계의 **쿼터 비용**이 실측으로 드러남: 요청당 분석 호출 +1. 분석기는 8b(별도 TPM)라 생성 예산과 분리돼 있으나, 전체 처리량이 늘며 생성 백엔드 소진이 빨라짐. 트래픽 증가 시 티어 상향 검토 필요.
