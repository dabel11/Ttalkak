...
(function attachMakeMessageModel(global) {
  ...
  const migrateV0ToV1 = (raw = {}) => ({ ...raw, schemaVersion: 1, mode: String(raw.mode || raw.type || "").toLowerCase() === "question" ? "ask" : raw.mode });
  const migrateV1ToV2 = (raw = {}) => ({ ...raw, schemaVersion: 2, improvedPrompt: raw.improvedPrompt || raw.improved_prompt || raw.finalPrompt || "" });
  function runMessageMigrations(raw = {}) { let value = { ...raw }; let version = Number(value.schemaVersion || 0); if (version < 1) { value = migrateV0ToV1(value); version = 1; } if (version < 2) value = migrateV1ToV2(value); return value; }
  ...
  function migratePersistedMakeState(state = {}) { state.messages = migrateMakeMessages(state.messages); state.recentThreads = (Array.isArray(state.recentThreads) ? state.recentThreads : []).map((thread) => ({ ...thread, messages: migrateMakeMessages(thread?.messages) })); return state; }
  function buildImproveHistory(messages) { return migrateMakeMessages(messages).filter((message) => message.role === "user" || message.role === "assistant").map((message) => ({ role: message.role, content: text(message.role === "assistant" ? message.answer || message.content : message.content) })).filter((message) => message.content); }
  function isExecutableMessage(message) { const value = migrateMakeMessage(message); return value.mode !== "ask" && !(value.questions.length && !value.improvedPrompt) && Boolean(text(message?.executablePrompt || message?.finalPrompt || value.improvedPrompt || value.content)); }
